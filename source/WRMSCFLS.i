...
{
	blnr WinCE = ((gbk_targ_wcar == cur_targ)
		|| (gbk_targ_wc86 == cur_targ));
	...
	if (WinCE) {
		WriteCStrToDestFile(
			" /D _WIN32_WCE=$(CEVersion) /D \"$(CePlatform)\""
...
LOCALPROC WriteRCexeFlags(void)
{
	...
	if (WinCE) {
		WriteCStrToDestFile(
			" /d UNDER_CE=$(CEVersion) /d _WIN32_WCE=$(CEVersion)"
			" /d \"UNICODE\" /d \"_UNICODE\" /d \"$(CePlatform)\" /r");
		if (gbk_cpufam_arm == gbo_cpufam) {
			WriteCStrToDestFile(
				" /d \"ARM\" /d \"_ARM_\" /d \"ARMV4\"");
		} else {
...
LOCALFUNC tMyErr WriteMSVCSpecificFiles(void)
{
	tMyErr err;

	if (noErr == (err = WriteADestFile(&OutputDirR,
		vStrAppAbbrev, MSVCWorkspaceExt(), WriteMSVCWorkSpaceFile)))
	if (noErr == (err = WriteADestFile(&OutputDirR,
		vStrAppAbbrev, MSVCProjectExt(), WriteMSVCProjectFile)))
	{
		/* ok */
	}
...
{
	vCheckWriteDestErr(DoAllSrcFilesWithSetup(DoSrcFileMSVCXMLAddFile));
}
...
LOCALFUNC tMyErr WriteMSVCXMLSpecificFiles(void)
{
	tMyErr err;

	if (noErr == (err = WriteADestFile(&OutputDirR,
		vStrAppAbbrev, ".sln", WriteMSVCXMLSolutionFile)))
	if (noErr == (err = WriteADestFile(&OutputDirR,
		vStrAppAbbrev, ".vcproj", WriteMSVCXMLProjectFile)))
	{
		/* ok */
	}
...
LOCALFUNC tMyErr WriteNMakeSpecificFiles(void)
{
	return WriteADestFile(&OutputDirR,
		"MAKEFILE", "", WriteNMakeMakeFile);
}
...
LOCALPROC WriteMSVCXML10ProjectFile(void)
{
	...
	++DestFileIndent;
		...
		WriteBeginXMLtagLine("ItemGroup");
			vCheckWriteDestErr(DoAllSrcFilesWithSetup(
				DoSrcFileMSVC10XMLAddHeaderFile));
			vCheckWriteDestErr(DoAllExtraHeaders2WithSetup(
...
LOCALFUNC tMyErr WriteMSVCXML10SpecificFiles(void)
{
	tMyErr err;
	...
	if (noErr == (err = WriteADestFile(&OutputDirR,
		vStrAppAbbrev, ".sln", WriteMSVCXMLSolutionFile)))
	if (noErr == (err = WriteADestFile(&OutputDirR,
		vStrAppAbbrev, ".vcxproj", WriteMSVCXML10ProjectFile)))
	if (noErr == (err = WriteADestFile(&OutputDirR,
		vStrAppAbbrev, ".vcxproj.filters",
...
LOCALFUNC tMyErr WriteMsvSpecificFiles(void)
{
	tMyErr err;

	if (UseCmndLine) {
		err = WriteNMakeSpecificFiles();
	} else {
		if (ide_vers >= 10000) {
			err = WriteMSVCXML10SpecificFiles();
		} else if (ide_vers >= 7000) {
			err = WriteMSVCXMLSpecificFiles();
		} else {
			err = WriteMSVCSpecificFiles();
		}
	}
