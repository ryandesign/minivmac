...
LOCALPROC PmuCheckCommandOp(void)
{
	switch (PMU_CurCommand) {
			...
			if (kPMUStateRecievingBuffer == PMUState) {
				if (0 == PMU_i) {
					if (PMU_BuffL >= 2) {
						...
						ReportAbnormal("PMU_BuffL too small for kPMUxPramWrite");
					}
				} else if (2 == PMU_i) {
		...
		case 0xEC: /* kPMUPmgrSelfTest - run the PMGR selftest */
			...
			break;
		case 0x78: /* kPMUreadINT - get PMGR interrupt data */
		case 0x68: /* kPMUbatteryRead - read battery/charger level and status */
		case 0x7F: /* kPMUsleepReq - put the system to sleep (sleepSig='MATT') */
			if (kPMUStateRecievedCommand == PMUState) {
				PMU_BuffA[0] = 0;
		...
		case 0x3A: /* kPMUxPramRead - read extended PRAM byte(s) */
			if (kPMUStateRecievedCommand == PMUState) {
				if ((2 == PMU_BuffL) && (PMU_BuffA[0] + PMU_BuffA[1] <= 0x80)) {
					PMU_p = &PARAMRAM[PMU_BuffA[0]];
					PMU_rem = PMU_BuffA[1];
			...
			break;
		case 0x38: /* kPMUtimeRead - read the time from the clock chip */
			if (kPMUStateRecievedCommand == PMUState) {
				if (0 == PMU_BuffL) {
					PMU_BuffA[0] = 0;
					PMU_BuffA[1] = 0;
			...
			break;
		case 0x31: /* kPMUpramWrite - write the original 20 bytes of PRAM (Portable only) */
			if (kPMUStateRecievedCommand == PMUState) {
				if (20 == PMU_BuffL) {
					/* done */;
				} else {
			...
			break;
		case 0x39: /* kPMUpramRead - read the original 20 bytes of PRAM (Portable only) */
			if (kPMUStateRecievedCommand == PMUState) {
				if (0 == PMU_BuffL) {
...
#if MakeDumpFile
				DumpACStr("Unknown PMU op ");
				DumpAHex(PMU_CurCommand);
				DumpANewLine();
				DumpACStr("PMU_BuffL = ");
				DumpAHex(PMU_BuffL);
				DumpANewLine();
		...
		case kPMUStateRecievingBuffer:
			if (! PmuToReady) {
				...
				if (nullpr == PMU_p) {
					...
					ReportAbnormal("PMU_p null while kPMUStateRecievingBuffer");
				}
				*PMU_p++ = v;
				--PMU_rem;
				++PMU_i;
