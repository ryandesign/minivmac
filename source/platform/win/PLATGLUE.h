					...
					if (SUCCEEDED(hres)) {
						...
						if (SUCCEEDED(hres)) {
							...
							*/
							...
							if (NULL != directory) {
								*directory = (0 != (wfd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY));
							}
							IsOk = trueblnr;
						}
					}
...
LOCALFUNC blnr MyFileExists(LPTSTR pathName, blnr *directory)
{
	...
	if (hf != INVALID_HANDLE_VALUE) {
		if (NULL != directory) {
			*directory = (0 != (fd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY));
		}
		IsOk = trueblnr;
		FindClose(hf);
	}
...
LOCALFUNC tMacErr ResolveNamedChild(LPTSTR pathName, char *Child, blnr *directory)
{
...
LOCALFUNC blnr ResolveNamedChildDir(LPTSTR pathName, char *Child)
{
	...
	return (mnvm_noErr == ResolveNamedChild(pathName, Child, &directory))
		&& directory;
}
...
LOCALFUNC blnr AllocMyMemory(void)
{
	...
	ReserveAllocBigBlock = (ui3p)GlobalAlloc(GMEM_FIXED | GMEM_ZEROINIT, n);
