...
#if MyAppIsBundle
LOCALFUNC blnr MyGetHomeDir(MyDir_R *d)
{
	...
	if (CheckSysErr(GetProcessBundleLocation(&currentProcess,
		&fsRef)))
	if (CheckSysErr(FSGetCatalogInfo(&fsRef, kFSCatInfoNone,
		NULL, NULL, &fsSpec, NULL)))
	{
		d->VRefNum = fsSpec.vRefNum;
		d->DirId = fsSpec.parID;
		return trueblnr;
	}
	return falseblnr;
}
...
#ifndef MayHaveMnvmExtns
#define MayHaveMnvmExtns 1
#endif
...
{
	if (FindSubDirectory(&SourceDir0R, &BaseDirR, "source", ""))
	if (FindSubDirectory(&SourceDirR, &SourceDir0R, "app", ""))
