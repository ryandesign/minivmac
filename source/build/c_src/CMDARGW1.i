...
#if CALL_NOT_IN_CARBON
#define MySys_GetWindPortRect(window, r) *(r) = (((WindowPeek)(window))->port.portRect)
#else
#define MySys_GetWindPortRect(window, r) GetPortBounds(GetWindowPort(window), r)
#endif

#if CALL_NOT_IN_CARBON
#define My_GetWindowKind(window) (((WindowPeek)(window))->windowKind)
#else
#define My_GetWindowKind GetWindowKind
#endif
...
#else
#define My_SetControlVisibilityFalse(inControl) (void) SetControlVisibility(inControl, false, false)
#endif
...
#else
#define My_SetControlVisibilityTrue(inControl) (void) SetControlVisibility(inControl, true, false)
#endif

#if CALL_NOT_IN_CARBON
#define My_GetControlBounds(control, r) *(r) = (**(control)).contrlRect
#else
#define My_GetControlBounds GetControlBounds
#endif

#if CALL_NOT_IN_CARBON
#define MySys_GetWindowPort(window) (window)
#else
#define MySys_GetWindowPort GetWindowPort
#endif

#if CALL_NOT_IN_CARBON
#define MySys_InvalRectInCurrentWindow(window, r) InvalRect(r)
#else
#define MySys_InvalRectInCurrentWindow InvalWindowRect
#endif

#if CALL_NOT_IN_CARBON
#define MySys_InvalRgnInCurrentWindow(window, r) InvalRgn(r)
#else
#define MySys_InvalRgnInCurrentWindow InvalWindowRgn
#endif

#if CALL_NOT_IN_CARBON
#define My_EnableMenuItem EnableItem
#else
#define My_EnableMenuItem EnableMenuItem
#endif

#if CALL_NOT_IN_CARBON
#define My_DisableMenuItem DisableItem
#else
#define My_DisableMenuItem DisableMenuItem
#endif
...
LOCALPROC GetWindowPortGlobalOffset(WindowPtr w, Point *p)
{
#if CALL_NOT_IN_CARBON
...
#else
	(void)GetWindowRegion(gMyMainWindow, kWindowUpdateRgn, localRgn);
#endif
...
LOCALPROC DrawTheLines(void)
{
	...
	MySys_GetWindPortRect(gMyMainWindow, &r);
	r.left = r.right - 15;
	r.right = r.left + 1;
	PaintRect(&r);
...
LOCALFUNC Boolean IsDAWindow(WindowPtr window)
{
	if (window == nil) {
		...
		return My_GetWindowKind(window) < 0;
	}
}
...
LOCALPROC MyDeskAccClose(WindowPtr w)
{
	SelectWindow(w);
	My_SetPortWindowPort(w);
	CloseDeskAcc(My_GetWindowKind(w));
}
#endif
...
{
	MySys_GetWindPortRect(gMyMainWindow, teRect);
...
LOCALPROC AdjustCursor(Point mouse, RgnHandle region)
{
	...
	if ((! gInBackground) && (! IsDAWindow(window))) {
		...
		if (window == gMyMainWindow) {
			...
			My_SetPortWindowPort(window); /* make a global version of the viewRect */
...
#else
			GetPortVisibleRegion(MySys_GetWindowPort(window), GlobalVisRgn);
#endif
...
LOCALPROC DoUpdateMenus(void)
{
	...
	if (undo) {
		My_EnableMenuItem(menu, kEditUndoItem);
	} else {
		My_DisableMenuItem(menu, kEditUndoItem);
	}
	if (cutCopyClear) {
		My_EnableMenuItem(menu, kEditCutItem);
		My_EnableMenuItem(menu, kEditCopyItem);
		My_EnableMenuItem(menu, kEditClearItem);
	} else {
		My_DisableMenuItem(menu, kEditCutItem);
		My_DisableMenuItem(menu, kEditCopyItem);
		My_DisableMenuItem(menu, kEditClearItem);
	}
	if (paste) {
		My_EnableMenuItem(menu, kEditPasteItem);
	} else {
		My_DisableMenuItem(menu, kEditPasteItem);
	}
	My_EnableMenuItem(menu, kEditSelectAllItem);
}
...
static pascal void MyTrackActionProc(ControlHandle control, short part)
{
#if ! TEWidthFromWindow
...
#if useUPP
LOCALVAR ControlActionUPP MyTrackAction = NULL;

LOCALFUNC blnr GetMyTrackAction(void)
{
	if (MyTrackAction == NULL) {
		MyTrackAction = NewControlActionUPP(MyTrackActionProc);
	}
	return MyTrackAction != NULL;
}
#else
#define MyTrackAction ((ProcPtr)MyTrackActionProc)
#define GetMyTrackAction() trueblnr
#endif
	...
	if (PtInRect(mouse, &teRect)) {
		...
		switch (part) {
			...
			default: /* they clicked in an arrow, so track & scroll */
				if (GetMyTrackAction()) {
					value = TrackControl(control, mouse, MyTrackAction);
				}
				break;
		}
	}
...
LOCALPROC ResizeMyWindow(void)
{
	...
	AdjustTE();
	MySys_GetWindPortRect(gMyMainWindow, &r);
	MySys_InvalRectInCurrentWindow(gMyMainWindow, &r);
}
...
#endif
			MySys_InvalRgnInCurrentWindow(gMyMainWindow, tempRgn); /* put back any prior update */
			DisposeRgn(tempRgn);
		}
	}
}
...
LOCALPROC DoActivate(Boolean becomingActive)
{
	...
	if (becomingActive) {
		...
		*/
		...
		My_GetControlBounds(MyDocVScroll, &r);
		MySys_InvalRectInCurrentWindow(gMyMainWindow, &r);
#if ! TEWidthFromWindow
		My_SetControlVisibilityTrue(MyDocHScroll);
		My_GetControlBounds(MyDocHScroll, &r);
		MySys_InvalRectInCurrentWindow(gMyMainWindow, &r);
#endif
		/* the growbox needs to be redrawn on activation: */
		MySys_GetWindPortRect(gMyMainWindow, &growRect);
		/* adjust for the scrollbars */
		growRect.top = growRect.bottom - kScrollbarAdjust;
		growRect.left = growRect.right - kScrollbarAdjust;
		MySys_InvalRectInCurrentWindow(gMyMainWindow, &growRect);
	} else {
...
LOCALPROC ProcessUpdateEvt(void)
{
	...
	GetPort(&saveport);
	My_SetPortWindowPort(MacEventWind);
...
#if useUPP
pascal static Boolean MyTEClickLoopProc(TEPtr theTE)
{
	Boolean v = InvokeTEClickLoopUPP(theTE, MyDocOldClik);
	PascalClikLoop();
	return v;
}
#endif
...
#if useUPP
LOCALVAR TEClickLoopUPP MyTEClickLoop = NULL;

LOCALFUNC blnr GetMyTEClickLoop(void)
{
	if (MyTEClickLoop == NULL) {
		MyTEClickLoop = NewTEClickLoopUPP(MyTEClickLoopProc);
	}
	return MyTEClickLoop != NULL;
}
#else
#define MyTEClickLoop ((TEClickLoopUPP)AsmClikLoop)
#define GetMyTEClickLoop() trueblnr
#endif
...
LOCALFUNC blnr MyMainWindowInit(void)
{
	...
	if (NULL != window) {
		...
		if (GetMyTEClickLoop()) {
			(*MyDocTE)->clickLoop = MyTEClickLoop;
		}
	...
	if (menuBar == nil) {
...
#if ! CALL_NOT_IN_CARBON
		{
			...
			if ((Gestalt(gestaltMenuMgrAttr, &response) == noErr) && (response & gestaltMenuMgrAquaLayoutMask)) {
				...
				if (theMenu != NULL) {
					DeleteMenuItem(theMenu, kFileQuitItem);
					DeleteMenuItem(theMenu, kFileQuitItem - 1); /* seperator */
				}
			}
		}
#endif
