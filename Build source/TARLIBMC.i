...
/*
	...
	fact is used by tarlib_write_file_header, so don't change it!
*/
...
# define MAX_OCTAL_VAL_WITH_DIGITS(digits) \
	...
	/* if (value <= MAX_OCTAL_VAL_WITH_DIGITS (size - 1)) */ /* this test isn't working */
	where[--i] = (char)'\0';
...
LOCALFUNC blnr
tarlib_WriteBytes(MyPtr p, uimr L)
{
	blnr IsOk;
	tarlib_r *tp = (tarlib_r *)pDt;

	pDt = tp->SavepDt;
	IsOk = tp->WriteBytes(p, L);
	pDt = (MyPtr)tp;
...
LOCALFUNC blnr
tarlib_write_eot(void)
{
	pBytesClear(tarlib_gd()->buffrecord.buffer, tarlib_BLOCKSIZE);
	return tarlib_WriteBytes(tarlib_gd()->buffrecord.buffer, tarlib_BLOCKSIZE);
}
...
tarlib_write_file_header(void)
{
	...
	pBytesClear(tarlib_gd()->buffrecord.buffer, sizeof(union tarlib_block));

	n = PStrLength(tarlib_gd()->CurFileDosPath);
	if (n > NAME_FIELD_SIZE) {
		n = (si4r)NAME_FIELD_SIZE;
	}
	MyMoveBytes(&tarlib_gd()->CurFileDosPath[1], tarlib_gd()->buffrecord.header.name, n);
	tarlib_gd()->buffrecord.header.name[NAME_FIELD_SIZE - 1] = (char)'\0';
	...
	*/
	...
	MODE_TO_OCT (tarlib_gd()->CurrentFileMode & MODE_ALL, tarlib_gd()->buffrecord.header.mode);
#else
	MODE_TO_OCT (tarlib_gd()->CurrentFileMode, tarlib_gd()->buffrecord.header.mode);
#endif

	UID_TO_OCT (/* current_stat.st_uid */UID_NOBODY, tarlib_gd()->buffrecord.header.uid);
	GID_TO_OCT (/* current_stat.st_gid */GID_NOBODY, tarlib_gd()->buffrecord.header.gid);
	OFF_TO_OCT (tarlib_gd()->CurrentFileSize, tarlib_gd()->buffrecord.header.size);
	TIME_TO_OCT (tarlib_gd()->CurrentFileModTime, tarlib_gd()->buffrecord.header.mtime);

#if archive_format == V7_FORMAT
	/* buffrecord.header.[ug]name are left as the empty string. */
#else
#if archive_format == OLDGNU_FORMAT
	/* Overwrite buffrecord.header.magic and header.version in one blow. */
	...
	strncpy (tarlib_gd()->buffrecord.header.version, TVERSION, TVERSLEN);
#endif
#endif
	...
	MyMoveBytes((MyPtr)CHKBLANKS, (MyPtr)tarlib_gd()->buffrecord.header.chksum, sizeof (tarlib_gd()->buffrecord.header.chksum));

	sum = 0;
	p = tarlib_gd()->buffrecord.buffer;
	...
	*/
	uintmax_to_oct ((uintmax_t) sum, tarlib_gd()->buffrecord.header.chksum, 7);
...
LOCALFUNC blnr tarlib_OpenFile(void)
{
	return CatInfoOpenReadDF(&tarlib_gd()->cPB,
		&tarlib_gd()->ifile);
}

LOCALFUNC blnr
tarlib_ProcessOne0(void)
{
	blnr IsOk = falseblnr;
	...
	*/
	if (! tarlib_gd()->CurFileIsDirectory) {
		tarlib_gd()->CurrentFileModTime = MacFtime2UnixFtime(tarlib_gd()->cPB.hfileInfo.ioFlMdDat);
		tarlib_gd()->CurrentFileSize = tarlib_gd()->cPB.hfileInfo.ioFlLgLen;
		tarlib_gd()->CurrentFileMode = 0666;
	} else {
		tarlib_gd()->CurrentFileModTime = MacFtime2UnixFtime(tarlib_gd()->cPB.dirInfo.ioDrMdDat);
		tarlib_gd()->CurrentFileSize = 0;
		tarlib_gd()->CurrentFileMode = 0777;
	}

	if (tarlib_write_file_header()) {
		if (tarlib_gd()->CurFileIsDirectory) {
			IsOk = trueblnr;
		} else {
			ui5r bufsize;
...
doGetMore:
				if (0 == sizeleft) {
					...
					if (sizeleft < tarlib_BLOCKSIZE) {
						...
						bufsize = sizeleft;
						pBytesClear(tarlib_gd()->buffrecord.buffer + sizeleft,
						(ui5r) (tarlib_BLOCKSIZE - sizeleft));
					} else {
						bufsize = tarlib_BLOCKSIZE;
					}
					if (MyReadBytes(tarlib_gd()->ifile,
						tarlib_gd()->buffrecord.buffer, bufsize))
					if (tarlib_WriteBytes(tarlib_gd()->buffrecord.buffer,
						tarlib_BLOCKSIZE))
					{
						sizeleft -= bufsize;
						goto doGetMore;
				...
				(void) MyCloseFile(tarlib_gd()->ifile);
			}
		}
	}
...
LOCALFUNC blnr tarlib_ProcessOne(void)
{
	blnr IsOk = falseblnr;
	int OldDosPathLen = PStrLength(tarlib_gd()->CurFileDosPath);

	tarlib_gd()->CurFileIsDirectory = CatInfoIsFolder(&tarlib_gd()->cPB);
	PStrAppend(tarlib_gd()->CurFileDosPath, tarlib_gd()->cPB.hfileInfo.ioNamePtr);
	if (tarlib_gd()->CurFileIsDirectory) {
		PStrApndChar(tarlib_gd()->CurFileDosPath, '/');
	}

	if (tarlib_ProcessOne0()) {
		if (! tarlib_gd()->CurFileIsDirectory) {
			IsOk = trueblnr;
		} else {
			blnr FinishOk;
			...
			while (MyCatGetNextChild(&tarlib_gd()->cPB,
...
GLOBALFUNC blnr tarlib_WriteTarFile(MyDir_R *d,
	ps3p s, ptWriteBytes p)
{
	MyPStr itemName;
	tarlib_r r;
	blnr IsOk = falseblnr;
...
LOCALFUNC blnr tarlib_WriteTarFile2_filter(MyPtr p, uimr L)
{
	short *refnum = (short *)pDt;

	return MyWriteBytes(*refnum, p, L);
}

GLOBALFUNC blnr tarlib_WriteTarFile2(
	...
	MyDir_R *dst_d, StringPtr dst_s)
{
		...
		'MPS ', 'TEXT', &refnum))
	{
		SavepDt = pDt;
		pDt = (MyPtr)&refnum;
		IsOk = tarlib_WriteTarFile(src_d, src_s,
			tarlib_WriteTarFile2_filter);
		pDt = SavepDt;
