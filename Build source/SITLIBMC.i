...
static ui4b updcrc(ui4b icrc, ui3b *icp, int icnt)
{
...
static blnr sitlib_copy(short fdin, short fdout, uimr L, ui4b *crc)
{
	uimr n;
	uimr maxn;
	MyPtr p;
...
static blnr sitlib_BeginProcessing(void)
/* write blank header that will rewrite later */
{
	...
	return MyWriteBytes(sitlib_gd()->ofd, (MyPtr)&sh, 22);
}


static blnr sitlib_EndProcessing(int numDocs)
{
	struct sitlib_sitHdr sh;
	long fpos2;
	blnr IsOk = falseblnr;

	if (CheckSysErr(GetFPos(sitlib_gd()->ofd, &fpos2))) {
		pBytesClear((MyPtr)&sh, sizeof(sh));
...
static blnr sitlib_ProcessRF(ui4b *f_crc)
{
	short fd;
...
static blnr sitlib_ProcessDF(ui4b *f_crc)
{
	short fd;
...
static blnr sitlib_ProcessOne0(long fpos1)
{
...
static blnr sitlib_PostProcessFolder(long fpos1,
	ui5b old_uncomp_tot)
{
	long fpos2;
	struct sitlib_fileHdr fh;
...
static blnr sitlib_ProcessOne(void)
{
	long fpos1;
	blnr IsOk = falseblnr;

	if (CheckSysErr(GetFPos(sitlib_gd()->ofd, &fpos1)))
	if (sitlib_ProcessOne0(fpos1))
	{
		if (! CatInfoIsFolder(&sitlib_gd()->cPB)) {
			IsOk = trueblnr;
		} else {
			...
			while (MyCatGetNextChild(&sitlib_gd()->cPB,
				&old_d, &index, &FinishOk)
				&& sitlib_ProcessOne())
			{
			}
			if (FinishOk) {
				IsOk = sitlib_PostProcessFolder(fpos1, old_uncomp_tot);
			}
		}
	}
...
GLOBALFUNC blnr sitlib_WriteFile(
	...
	MyDir_R *dst_d, StringPtr dst_s)
{
		...
		'SIT!', 'SIT!', &r.ofd))
	{
		...
		if (MyFileGetCatInfo(src_d, src_s,
			itemName, &r.cPB))
		if (sitlib_BeginProcessing())
		if (sitlib_ProcessOne())
		if (sitlib_EndProcessing(1))
		{
			IsOk = trueblnr;
		}
